...
bash# Complete modular setup with all v5.0 features
...
bash# Complete enhanced setup with modular architecture
curl -fsSL https://raw.githubusercontent.com/hue-studios/directus-newsletter-installer/main/deploy.sh | bash -s full \
  https://admin.yoursite.com admin@example.com password https://yoursite.com
...
bash# 1. Setup modular environment
...
bash# 1. Download the modular deployment script
...
Install Command
bash./deploy.sh install <directus-url> <email> <password> [frontend-url] [webhook-secret]
...
Frontend Command
bash./deploy.sh frontend [nuxt-project-path]
...
Flow Command
bash./deploy.sh flow <directus-url> <email> <password> <frontend-url> [webhook-secret]
...
Debug Command
bash./deploy.sh debug <directus-url> <email> <password> [frontend-url]
...
Fix Flow Command
bash./deploy.sh fix-flow <directus-url> <email> <password>
...
bash# Create backup using included script (if available)
./deploy.sh backup
...
bash# This installs all v5.0 features with modular architecture
curl -fsSL https://raw.githubusercontent.com/hue-studios/directus-newsletter-installer/main/deploy.sh | bash -s full \
...
bash# Setup modular environment
./deploy.sh setup
...
bash# Complete frontend integration package
...
bashcd /path/to/your/nuxt/project
...
bash# Start your Nuxt development server
npm run dev
...
Enhanced Newsletter Validation - Checks templates, categories, and all required fields
Enhanced MJML Compilation - Supports new field structure and individual block fields
Enhanced Data Retrieval - Gets newsletter and mailing list data efficiently
Enhanced Send Record Creation - Tracks A/B testing, analytics, and performance metrics
Enhanced Email Sending - Advanced personalization, batching, and delivery tracking
Enhanced Status Updates - Updates analytics, performance metrics, and completion status
Enhanced Success Logging - Comprehensive logging with category and performance data

Manual Flow Verification and Fixing
bash# Debug the complete installation
./deploy.sh debug https://admin.yoursite.com admin@example.com password https://yoursite.com
...
Go to Content → Newsletters → Create New Newsletter
Verify "Blocks" section exists and is functional
Try adding blocks using the + button
Drag and reorder blocks to test sorting
Edit individual block fields (title, subtitle, etc.)

Test 2: Test Template System

Go to Content → Newsletter Templates
Verify sample templates exist
Create a new newsletter
Select a template from dropdown
Verify blocks are pre-populated from template

Test 3: Test Enhanced Compilation
bash# Create a newsletter with enhanced features, then:
...
Create a newsletter with enhanced features
Add several blocks using individual fields
Set category, tags, and priority
Select template and mailing list
Set status to "Ready to Send"
Click "Send Newsletter" button
Verify enhanced flow execution
Check analytics and performance updates
...
Create reusable templates with pre-configured blocks
Set default sender information and subject patterns
Use templates when creating new newsletters for faster creation
Track template usage and effectiveness over time
...
Create reusable content blocks (headers, footers, call-to-actions)
Organize content with categories and tags for easy discovery
Insert saved content when building newsletters for consistency
Share content across team members for brand consistency
...
Import subscribers with enhanced data and preferences
Set up preference categories for newsletter types
Track engagement and behavior patterns
Segment based on preferences, engagement, and custom fields
...
Create newsletters and add blocks using the "Blocks" section
Use individual fields for better UX (no more complex JSON editing)
Drag blocks to reorder and create perfect newsletter layouts
Edit block content directly in the admin interface
...
Enable A/B testing on newsletters
Set test percentage (5-50% of audience)
Create alternative subject lines or content
Track performance of both versions
Use insights to optimize future newsletters
...
Submit newsletters for team review
Reviewers can approve/reject with detailed notes
Track approval history and workflow
Automate sending upon final approval
...
Solutions:
bash# Fix relationships specifically
./deploy.sh fix-flow https://admin.yoursite.com admin@example.com password
...
Solutions:
bash# Check frontend integration
...
javascript// Enhanced template with dynamic variables
{
...
sql-- High engagement B2B subscribers interested in product updates
...
sql-- Performance by newsletter category and template
SELECT 
  n.category,
...
Enhanced Flow Setup Guide - Detailed flow configuration
Enhanced Troubleshooting Guide - Common issues and solutions
Frontend Integration Guide - Complete frontend setup

🛠️ Debug Tools
bash# Comprehensive system debugging
